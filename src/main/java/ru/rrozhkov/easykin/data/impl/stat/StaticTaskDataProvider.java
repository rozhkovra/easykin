...
public class StaticTaskDataProvider extends CollectionDataProvider<ITask>{
	public StaticTaskDataProvider(){
		...
		TaskFactory.createTask(
				...
				, new Date(), DateUtil.parse("31.08.2016"), Priority.IMPOTANT_NOFAST, 1, null, Status.OPEN)
		,TaskFactory.createTask(
				"Купить зимнюю резину Cordiant"
				, new Date(), DateUtil.parse("31.08.2016"), Priority.IMPOTANT_NOFAST, 4, null, Status.CLOSE)
		,TaskFactory.createTask(
				"Сдать 1050 на методические пособия Лешке"
				, new Date(), DateUtil.parse("31.05.2016"), Priority.IMPOTANT_FAST, 3, null, Status.CLOSE)
		,TaskFactory.createTask(
				"Замена тормозных шлангов"
				, new Date(), DateUtil.parse("30.05.2016"), Priority.IMPOTANT_FAST, 4, null, Status.OPEN)
		,TaskFactory.createTask(
		...
		,TaskFactory.createTask(
				...
				, new Date(), DateUtil.parse("30.05.2016"), Priority.SIMPLE, 4, null, Status.OPEN)
		,TaskFactory.createTask(
				"Оплатить пеню за транспортный налог"
				, new Date(), DateUtil.parse("30.05.2016"), Priority.IMPOTANT_NOFAST, 4, null, Status.CLOSE)
		,TaskFactory.createTask(
				"Замена правой передней шаровой"
				, new Date(), DateUtil.parse("30.03.2016"), Priority.IMPOTANT_FAST, 4, null, Status.CLOSE)
		,TaskFactory.createTask(
				"Замена резины 800 р"
				, new Date(), DateUtil.parse("30.03.2016"), Priority.IMPOTANT_NOFAST, 4, null, Status.CLOSE))));
	}
}